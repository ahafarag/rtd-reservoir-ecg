    ...
    if os.path.exists(ptbxl_root):
        try:
            ...
            selected_lead = st.selectbox(
                "Select ECG Lead", lead_names, index=lead_names.index(cfg.get("lead", lead_names[0])), key="lead"
            )
            ...
            ecq_file = df
        except Exception as e:  # pragma: no cover - user provided paths may fail
...
if ecq_file is not None:
    ...
    use_mlp = (
        st.sidebar.radio(
            "Readout Type", ["MLP (Neural Network)", "Ridge Regression"], index=0 if cfg.get("use_mlp") else 1, key="use_mlp"
        )
        == "MLP (Neural Network)"
    )
    ...
    if st.button("Run Simulation"):
        ...
        signal_range = np.max(y_true_tot) - np.min(y_true_tot) or 1.0
...
"""
        ...
        result_df = pd.DataFrame({"True ECG": model.Y_true, "Predicted ECG": model.Y_pred})
